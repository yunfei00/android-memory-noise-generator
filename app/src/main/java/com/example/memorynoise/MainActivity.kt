...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun row(title: String, choices: List<String>, value: String): Pair<Spinner,EditText> {
            ...
            val spinner=Spinner(this).apply { adapter=ArrayAdapter(this@MainActivity,android.R.layout.simple_spinner_dropdown_item,choices) }; line.addView(spinner,LinearLayout.LayoutParams(0,dp(46),1f))
            val edit=EditText(this).apply { setText(value); textSize=14f; inputType=android.text.InputType.TYPE_CLASS_NUMBER; isSingleLine=true; visibility=View.GONE }; line.addView(edit,LinearLayout.LayoutParams(dp(90),dp(46)))
            ...
            controls.add(spinner); controls.add(edit); root.addView(line); return spinner to edit
        }
        ...
        customHex=EditText(this).apply { hint="CUSTOM HEX，例如 DEADBEEF"; setText("AA55"); isSingleLine=true; textSize=14f; visibility=View.GONE }; root.addView(customHex); controls.add(customHex)
        ...
        val durationRow=root.childCount; row("Duration",listOf("Unlimited","10 s","30 s","60 s","120 s","300 s","Custom"),"10").also { duration=it.first; customDuration=it.second }
        val writeRow=root.childCount; row("Write ms",listOf("100","500","1000","2000","5000","Custom"),"1000").also { write=it.first; customWrite=it.second; write.setSelection(2) }
        val idleRow=root.childCount; row("Idle ms",listOf("100","500","1000","2000","5000","Custom"),"1000").also { idle=it.first; customIdle=it.second; idle.setSelection(2) }
        val sweepTimeRow=root.childCount; row("Sweep /组",listOf("10 s","20 s","30 s","60 s","Custom"),"20").also { sweepTime=it.first; customSweepTime=it.second; sweepTime.setSelection(1) }
        val sweepGapRow=root.childCount; row("Gap",listOf("0 s","1 s","2 s","5 s","Custom"),"2").also { sweepGap=it.first; customSweepGap=it.second; sweepGap.setSelection(2) }
        fun refreshMode(pos:Int) {
            val burst=pos==1; val sweep=pos==3
            root.getChildAt(durationRow).visibility=if(sweep) View.GONE else View.VISIBLE
            root.getChildAt(writeRow).visibility=if(burst) View.VISIBLE else View.GONE; root.getChildAt(idleRow).visibility=if(burst) View.VISIBLE else View.GONE
            root.getChildAt(sweepTimeRow).visibility=if(sweep) View.VISIBLE else View.GONE; root.getChildAt(sweepGapRow).visibility=if(sweep) View.VISIBLE else View.GONE
            pattern.visibility=if(sweep) View.INVISIBLE else View.VISIBLE; customHex.visibility=if(!sweep && pattern.selectedItemPosition==6) View.VISIBLE else View.GONE
        ...
        sweepStatus=label("",27f).apply { gravity=android.view.Gravity.CENTER; typeface=android.graphics.Typeface.DEFAULT_BOLD; setPadding(dp(4),dp(10),dp(4),dp(10)); visibility=View.GONE }
    ...
    private fun begin(){
        try {
            ...
            require(seconds in 0..86400){"实验时长必须为 1–86400 秒"}
    ...
    private fun render(){
        ...
        state.text=if(running&&stopping) "STOPPING…" else s.state; state.setBackgroundColor(when{ s.error!=null->Color.rgb(160,35,35); running&&s.state.endsWith("WRITE")->Color.rgb(0,112,75); running->Color.rgb(156,96,0); else->Color.rgb(65,75,85) })
        val isSweep=s.config?.mode==Mode.SWEEP; sweepStatus.visibility=if(isSweep&&running) View.VISIBLE else View.GONE
        if(isSweep){ val pos=(s.sweepIndex+1).coerceAtLeast(1); sweepStatus.text="# $pos / ${s.sweepTotal}\n${s.sweepCurrent}\n剩余 ${String.format(Locale.US,"%.1f",s.sweepRemainingMs/1000.0)} s\nNext  ${s.sweepNext}" }
